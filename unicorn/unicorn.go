...
func NewUnicorn(arch, mode int) (*Unicorn, error) {
	...
	u := &Unicorn{handle: handle, hooks: make(map[Hook]uintptr)}
	// TODO(felber) call Close from a defer function
	runtime.SetFinalizer(u, func(u *Unicorn) { u.Close() })
	return u, nil
}
...
func (u *Unicorn) StartWithOptions(begin, until uint64, options *UcOptions) error {
...
func (u *Unicorn) RegWriteBatch(regs []int, vals []uint64) error {
	if len(regs) == 0 {
		return nil
	}
...
func (u *Unicorn) MemRegions() ([]*MemRegion, error) {
...
func (u *Unicorn) MemReadInto(dst []byte, addr uint64) error {
	if len(dst) == 0 {
		return nil
	}
...
func (u *Unicorn) MemMapPtr(addr, size uint64, prot int, ptr unsafe.Pointer) error {
