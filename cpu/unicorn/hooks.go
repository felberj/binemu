...
func (u *Cpu) HookBlock(cb CodeHook, start, end uint64) (cpu.Hook, error) {
...
func (u *Cpu) HookCode(cb CodeHook, start, end uint64) (cpu.Hook, error) {
	wrap := func(_ uc.Unicorn, addr uint64, size uint32) { cb(addr, size) }
	return u.Unicorn.HookAdd(cpu.HOOK_CODE, wrap, start, end)
}

// HookMem TODO figure out what it does
func (u *Cpu) HookMem(kind int, cb MemHook, start, end uint64) (cpu.Hook, error) {
	kind &= cpu.HOOK_MEM_READ | cpu.HOOK_MEM_WRITE
	if kind == 0 {
		return nil, errors.New("invalid hook type")
	}
...
func (u *Cpu) HookMemErr(kind int, cb MemErrHook, start, end uint64) (cpu.Hook, error) {
...
func (u *Cpu) HookInterrupt(cb InterruptHook, start, end uint64) (cpu.Hook, error) {
